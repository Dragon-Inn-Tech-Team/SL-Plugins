...
namespace CustomCommands.Features.CustomEvents.Events
{
	public class SCPInfection : CustomEventRound
	{
		public override void OnServerRoundStarted()
		{
			...
			int scps = Mathf.Clamp((int)Math.Floor((double)(Player.Count / 5)), 1, 10);
		...
		public void SpawnPlayerAsHuman(Player player)
		{
			player.SetRole(PlayerRoles.RoleTypeId.NtfSergeant);
			player.ClearInventory();

			player.AddItem(ItemType.ArmorHeavy);
			player.AddFirearmWithAttachments(ItemType.GunShotgun, AttachmentName.HoloSight, AttachmentName.Flashlight);
			player.AddAmmo(ItemType.Ammo12gauge, 74);
			player.AddFirearmWithAttachments(ItemType.GunCOM18, AttachmentName.HoloSight, AttachmentName.Flashlight);
			player.AddAmmo(ItemType.Ammo9x19, 210);
		}
